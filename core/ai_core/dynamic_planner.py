...
def plan_actions(intent_data):
    ...
    tabla_candidates = {
        "usuario": ["colaborador", "empleado", "persona", "usuario", "trabajador"],
    ...
    equipo = None
    # --- Detección de filtro por equipo (solo si se indica claramente)
    match_equipo = re.search(r"equipo\s+(?:llamado|denominado|con nombre)?\s*(\w+)", texto)

    # Aplicar filtro solo si el usuario menciona "equipo <nombre>" explícitamente
    if match_equipo:
        equipo_detectado = match_equipo.group(1)
        plan["filtros"]["equipo"] = equipo_detectado
    else:
        # No aplicar filtro por defecto
        plan["filtros"]["equipo"] = None
